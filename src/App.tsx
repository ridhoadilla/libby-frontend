...
import { signInWithRedirect, getRedirectResult, signOut, onAuthStateChanged, User as FirebaseUser, signInWithEmailAndPassword } from 'firebase/auth';
...
export default function App() {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      const code = getHttpErrorCode(error.code);
      setAuthError({ code, message: error.message || "Gagal menginisiasi proses masuk dengan Google." });
    }
  };
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const code = getHttpErrorCode(error.code);
      setAuthError({ code, message: error.message || "Gagal masuk. Periksa kembali email dan password Anda." });
    } finally {
      ...
      <nav className="sticky top-0 z-50 bg-warm-bg/80 backdrop-blur-md border-b border-gray-200 px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          {user ? (
            ...
            <button onClick={() => setIsLoginModalOpen(true)} className="flex items-center gap-2 bg-olive text-white px-4 py-2 rounded-full text-sm font-sans font-medium hover:opacity-90 transition-opacity">
      ...
      <AnimatePresence>
        {isLoginModalOpen && (
          ...
          >
            ...
            >
              <div className="absolute top-0 right-0 w-32 h-32 bg-olive/10 rounded-full translate-x-1/2 -translate-y-1/2 blur-2xl"></div>
              
              <div className="flex justify-between items-center mb-8 relative z-10">
                <h2 className="text-3xl font-medium">Masuk</h2>
                <button onClick={() => setIsLoginModalOpen(false)} className="p-2 text-gray-400 hover:text-gray-800 bg-gray-50 hover:bg-gray-100 rounded-full transition-colors">
                  <X className="w-5 h-5" />
                </button>
              </div>

              <form onSubmit={handleEmailLogin} className="space-y-5 relative z-10 font-sans">
                <div>
                ...
                <button 
                  ...
                  className="w-full bg-olive text-white py-4 rounded-xl font-bold hover:bg-olive/90 transition-all flex items-center justify-center gap-2 mt-4 disabled:opacity-70 shadow-md"
                >
                  {loginLoading ? (
              ...
              <div className="mt-8 relative z-10">
                <div className="relative flex items-center justify-center py-4">
                  ...
                  <div className="relative bg-white px-4 text-xs font-semibold text-gray-400 uppercase tracking-widest font-sans">Atau</div>
                </div>
                
                <button 
                  onClick={handleGoogleLogin} 
                  type="button"
                  className="w-full mt-2 bg-white border-2 border-gray-100 text-gray-700 py-4 rounded-xl font-bold hover:bg-gray-50 hover:border-gray-200 transition-all flex items-center justify-center gap-3 font-sans"
                >
        ...
        {currentView === 'home' ? (
          ...
          isAdmin ? (
            ...
            <div className="text-center py-32 px-6">
              ...
              <button onClick={() => { localStorage.setItem('intendedView', 'admin'); setIsLoginModalOpen(true); }} className="bg-olive text-white px-8 py-3 rounded-full font-sans font-medium hover:opacity-90 transition-opacity flex items-center gap-2 mx-auto">
